...
{
	UStaticMesh* mesh = FPakLoader::Get()->LoadObjectFromPak<UStaticMesh>(Filename);
	FString meshInfo = mesh->GetDesc();
...
ULevel* UPakLoaderLibrary::GetPakFileLevel(const FString& Filename)
{
	return FPakLoader::Get()->LoadObjectFromPak<ULevel>(Filename);
}

UMaterialInstance* UPakLoaderLibrary::GetPakFileMaterialInstance(const FString& Filename)
{
	return FPakLoader::Get()->LoadObjectFromPak<UMaterialInstance>(Filename);
}

FString UPakLoaderLibrary::GetIOSDocumentPakPath(const FString& Filename)
{
	FString ParamFilePath = FPaths::RootDir() + Filename;
